...
object UsersService {
  def make[F[_]: Sync: Calendar: MonadThrow](
      usersRepo: UsersRepository[F],
      peopleRepo: PeopleRepository[F],
    ): UsersService[F] = new UsersServiceImpl[F](usersRepo, peopleRepo)
}

final class UsersServiceImpl[F[_]: Sync: Calendar: MonadThrow](
    usersRepo: UsersRepository[F],
    peopleRepo: PeopleRepository[F],
